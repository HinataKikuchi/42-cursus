...
/*   Updated: 2021/02/08 20:23:44 by hkikuchi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		write_string(va_list ap, t_format *x)
{
	...
	if (x->minus_flag)
	{
		...
		x->word_count += (x->ac < s_len) ? write_blank(x->min - x->ac) : write_blank(x->min - s_len);
		return ;
	}
	else
	{
	...
	x->word_count += (s_len > x->ac && ft_strchr(x->format_num, '.')) ? write(1, s, x->ac) : write(1, s, s_len);
}
